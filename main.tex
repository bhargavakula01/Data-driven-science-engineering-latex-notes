...
\begin{itemize}
    \item Steps for Randomized Linear Algebra (specific to SVD). This works primarily for \textbf{tall-skinny matrices}, where $n < m$.
    \begin{enumerate}
        \item Identify a target rank, where $r < m$. \newline
        \textit{\textbf{Note:} The "rank" is the dimension of the column space.}

        \item Use random projections, $P$, to sample the column space and find a matrix $Q$ that can approximate the column space of $X$, so that $X \approx QQ^*X$
