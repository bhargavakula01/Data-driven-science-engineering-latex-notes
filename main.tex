...
\begin{itemize}
    ...
    \begin{itemize}
        \item This is done by extracting "low rank" (number of linearly independent rows or columns) approximations for a matrix and performing an inverse/pseudo-inverse to determine a solution to a system of equations in the form $Ax = b$
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \textit{The reason we use $n-1$ (known as \textbf{Bessel's Correction}) is because the above steps does not account for the variance of $\Bar{X}$ against the true mean.}
...
\begin{itemize}
    \item Steps for Randomized Linear Algebra (specific to SVD):
    \begin{enumerate}
        \item 
    \end{enumerate}
\end{itemize}
