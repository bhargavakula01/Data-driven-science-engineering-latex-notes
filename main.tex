...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Note:} A \textbf{Hilbert space} is a complete vector space that is equipped with an inner product. Hilbert spaces were originally used to study finite-dimensional vector spaces but have been expanded to study function spaces, essentially vector spaces of infinite dimension.
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item A bounded norm is where the norm is less than infinity:
        \begin{equation*}
            \Vert f \Vert_2 < \infty
        \end{equation*}
...
\begin{itemize}
    ...
    \end{equation*}
    \item After evaluating the limit from the above equations, $\left< f(x), \psi_k(x) \right>$ becomes the Fourier transform of the vector, denoted as the following ($\hat{f}(w) \triangleq \mathcal{F}(f(x))$). This leads to the following two equations, which are also known as the \textbf{Fourier transform pair}.
    \begin{align*}
    ...
    \item What are some applications of Fourier Transforms?:
    \begin{enumerate}
