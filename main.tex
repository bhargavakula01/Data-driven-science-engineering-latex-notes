...
\textbf{Date: } \today
\begin{center}
    \textbf{Notes on Data-Driven Science and Engineering}
\end{center}

\section{Singular Value Decomposition (SVD)}

\begin{itemize}
    \item \textbf{Definition of SVD:} Computational-based method of extracting patterns from high-dimensional data sets
    \begin{itemize}
        \item This is done by extracting "low rank" approximations for a matrix and performing an inverse/pseudo-inverse to determine a solution to a system of equations in the form $Ax = b$
        \begin{itemize}
            \item \textbf{Rank}: number of linearly independent columns or dimension of the column space
...
\begin{itemize}
 ...
 Below is the derivation:
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item the eigendecomposition of $A^*A$ allows one to determine $V$
        \item the eigendecomposition of $AA^*$ allows one to determine $U$
    ...
    \begin{itemize}
        ...
        \end{itemize}
        \item \textbf{From ChatGPT:} What is a Bidiagonal Matrix?
        \begin{itemize}
...
\begin{itemize}
    ...
    \item We can represent the matrix $X$ as a sum of rank 1 matrices (because $\Sigma$ is a diagonal matrix). Known as a \textit{diadic summation}. Furthermore, the singular values ($\sigma_k$) are arranged in decreasing order ($\sigma_1 \geq \sigma_2 \geq \sigma_3 \geq ... \geq \sigma_m$)
    \begin{equation*}
        X = \Sigma^m_{k=1} \sigma_ku_kv_k^*
    \end{equation*}
    ...
    \begin{conditions}
        ...
        \tilde{V} & first $r$ columns of matrix V (or first $r$ rows of $V^*$) \\
        \tilde{\Sigma} & $r$x$r$ sub-block of matrix $\Sigma$
    \end{conditions}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            argmin_{\tilde{X}, s.t rank(\tilde{X})=r} \Vert X - \tilde{X} \Vert_F = \tilde{U}\tilde{\Sigma}\tilde{V}^*
        \end{equation*}
        \begin{itemize}
            \item \textit{As the rank $r$ increases, $\tilde{X}$ will start to approach $X$}
            \item By minimizing the error, one will get to the truncated SVD
        \end{itemize}
        \item One can quantify the error of the Eckart-Young theorem:
        \begin{equation*}
            \Vert X - \tilde{X} \Vert^2_F = \Sigma^m_{k=r+1} \sigma^2_k 
        \end{equation*}
    ...
    \begin{equation*}
        argmin_{\tilde{X}, s.t rank(\tilde{X})=r} \Vert X - \tilde{X} \Vert_2 = \tilde{U}\tilde{\Sigma}\tilde{V}^*
    \end{equation*}
    \begin{itemize}
        \item The 2-norm of matrix X is induced by the vector 2-norm. This is to determine the maximum possible stretching that matrix X can apply to any unit vector. This is known as the spectral norm
        \begin{equation*}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
               ...
               \item $\|x\|_2 = \sqrt{\Sigma_{i=1}^n |x_i|^2}$
               \item Measures the length/magnitude of a vector
            ...
            \begin{itemize}
                \item Applies to matrices
                \item $\|A\|_F = \sqrt{\Sigma_{i=1}^{m} \Sigma_{j=1}^{n} |A_{ij}|^2}$
                \item One can generalize the Frobenius norm for an entrywise p-norm; however, the entrywise 2-norm is referred to as the "Frobenius Norm."
                \item \textit{Frobenius norm} is not a matrix norm but more of an entrywise norm for matrices.
            \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            X^*X &= V \begin{bmatrix} \hat{\Sigma} & 0 \end{bmatrix}U^*U \begin{bmatrix} \hat{\Sigma} \\ 0 \end{bmatrix}V^* \\
                &= V\hat{\Sigma}^2V^*\\
            X^*XV &= V\hat{\Sigma}^2 
        \end{align*}
        ...
        \textit{\textbf{What is Self Adjoint matrix?}(\href{https://en.wikipedia.org/wiki/Hermitian_matrix}{self-adjoint/Hermitian Matrix}): A self-adjoint matrix is one where $X = X^*$ or in the real space $X = X^T$.}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The trick is to perform eigendecomposition on the $X^*X$ matrix. This would enable one to determine the matrices $V$ and $\hat{\Sigma}$. If there are singular values that are equal to 0 in the matrix $\hat{\Sigma}$, then we would keep the $r$ non-zero part (which is the matrix $\tilde{\Sigma}$ from earlier) and the corresponding columns $\tilde{V}$ of $V$. From this, one can approximate the first r columns of U (which is $\tilde{U}$):
        \begin{equation*}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{equation*}
            XV = U\Sigma \Rightarrow X = U\Sigma V^*
        \end{equation*}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    Left Unitary Transformation of $X$:
    \begin{equation*}
        Y = CX = CU_X\Sigma_XV_X^*
    \end{equation*}

    Right Unitary Transformation of $X$:
    \begin{equation*}
        Y = XC^* = U_X\Sigma_XV_X^*C^*
    \end{equation*}
    \textit{Where $C$ is a unitary matrix}
    \begin{itemize}
        \item \textit{Page 16 in the textbook shows examples of left and right unitary transformations}
    \end{itemize}
    \item \textbf{(Extra notes)} Generic properties of Unitary transformation which make them useful:
    \begin{itemize}
        \item Preserves lengths (norms) and angles (inner products).
        \item Represents a rotation or reflection in complex vector spaces (orthogonal transformation in real vector spaces).
        \item Is invertible and stable numerically (no amplification of error from input vectors).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item In SVD, the column space of $A$ in the equation $A = \tilde{U}\tilde{\Sigma}\tilde{V}^*$ is equal to the column space of $\tilde{U}$ where $\tilde{U}$ is a truncated version of $U$ based on the rank of $A$.
        ...
        \begin{itemize}
            \item In SVD, the row space of $A$ (where $A = \tilde{U}\tilde{\Sigma}\tilde{V}^*$), is equal to the row space of $\tilde{V}^*$ or the column space of $\tilde{V}$. Similarly, $\tilde{V}$ is a truncated version of $V$ based on the rank of $A$.
            \item the orthogonal complement of $row(A)$ is $ker(A)$ (null space) and is derived from $col(\hat V^\perp$)
        \end{itemize}

        \item \textbf{Summary:} if $b \in col(A)$ and $dim(ker(A)) \neq 0$ then there is an infinite number of solutions, $x$. If $b \notin col(A)$ then there are no solutions. 
        
        Essentially, the $dim(ker(A))$ is $n - rank(A)$ where $n$ is the number of columns in $A$. If $dim(ker(A)) > 0$ then the number of variables is greater than constraints. If $dim(ker(A)) = 0$ then number of variables is equal to the number of constraints. 

        \item The fundamental subspaces follow the following equations (where $A$ is $\mathbb{R}^{m\times n}$):
        \begin{align*}
            col(A) \oplus ker(A^*) = \mathbb{R}^n \\
            col(A^*) \oplus ker(A) = \mathbb{R}^n
        \end{align*}
    \end{itemize}
    \item In an over-determined system where no solution exists, we generally like the find the solution $x$ that minimizes the sum of squared errors ($\Vert Ax - b \Vert_2^2 $). For an under-determined system where infinite solutions exist, we generally like to find the solution that minimizes the norm $\Vert x \Vert_2$ (aka. norm-minimization).
    \begin{itemize}
        \item This can be solved by using the \textbf{Moore-Penrose left Pseudoinverse}
        
        \textbf{Note:} the tilde is representing that this is part of the truncated SVD
        \begin{align*}
            A^{+} &\triangleq \tilde{V}\tilde{\Sigma}^{-1}\tilde{U}^*\\
            A^{+}A &= \tilde{V}\tilde{V}^*
        \end{align*}

        \textbf{Note:} Important to point out that the $A^{+}A$ (and $\tilde{V}\tilde{V}^*$) is only equal to identity if the truncated SVD captures all of the non-zero singular values. 
        
        The Moore-Penrose left Pseudoinverse can be used to determine the least-squares and minimum-norm solution
        \begin{align*}
            A^{+}A\tilde{x} &= A^{+}b \\
            \tilde{x} = \tilde{V}\tilde{\Sigma}^{-1}\tilde{U}^*b
        \end{align*}

        plugging it back into $Ax = b$
        \begin{align*}
            A\tilde{x} &= \tilde{U}\tilde{\Sigma}\tilde{V}^*\tilde{V}\tilde{\Sigma}^{-1}\tilde{U}^*b \\
            &= \tilde{U}\tilde{U}^*b
        \end{align*}

        \textbf{Note:} $\tilde{U}\tilde{U}^*$ does not equal the identity matrix (as it a truncated matrix and therefore doesn't span $\mathbb{R}^n$) but rather a projection onto the column space of \tilde{U}. $\tilde{x}$ will only be a exact solution if $b$ is in the column space of $\tilde{U}$ and therefore in the column space of $A$. On the other hand, $\tilde{U}^*\tilde{U} = I_{n\times n}$.
        \begin{itemize}
            \item \textit{An aside}: In general for a matrix, $Q$, with orthonormal columns:
            \begin{equation*}
                P = QQ^*
            \end{equation*}
            where $P$ is the orthogonal projection onto the column space of Q.
        \end{itemize}
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item represents how sensitive matrix multiplication and matrix inversion are to changes to the input \textit{(useful information in Model Predictive Control development)}.
        ...
        \begin{conditions}
            ...
            A & Input matrix
        \end{conditions}
        \item in APC development conditions value analysis is used in determining colinearities within the gain matrix. Near colinearities can cause cycling to occur within a MPC (model predictive controller) and thus can cause equipment damage at a plant.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \end{equation*}
        \item If we assume a worst case scenario when solving for $b$, where $\epsilon_x$ is aligned with the singluar vector corresponding to the max singular value, $\sigma_{max}$, and the input, $x$, is aligned with the singular vector corresponding to the minimum singular value, $\sigma_{min}$, one will get the following equation:
        \begin{equation*}
        ...
        \end{conditions}
        \item From the above equation, the output signal-to-noise ratio ($\frac{b}{\epsilon_b}$) is equal to $\frac{x}{\epsilon_x} \cdot \frac{\sigma_{min}}{\sigma_{max}}$. This shows that the output signal-to-noise ratio is degraded by a factor equal to the condition number (under the worst case scenario).
        \item If we assume the worst case scenario when solving for $x$ given an imperfect $b$, where $\epsilon_b$ is aligned with the singular vector corresponding to the max singular value $\sigma_{max}$ and $b$ is aligned with the singular vector corresponding to the min singular value $\sigma_{min}$
        \begin{equation*}
            x + \epsilon_x \approx A^{-1}\left( b + \epsilon_b \right) = \frac{1}{\sigma_{max}}b + \frac{1}{\sigma_{min}}\epsilon_b
        \end{equation*}

        As we can see here, the input signal-to-noise ratio (as we are solving for $x$ in this scenario) is degraded by a factor equal to the condition number.
    \end{itemize}
...
\begin{itemize}
    ...
    \item Example:
    \begin{itemize}
        \item One can use a pseudo inverse to determine the least-squares solution, as shown in the equation below (assuming that $a$ and $b$ are column vectors):
        \begin{align*}
            b &= ax = \tilde{U}\tilde{\Sigma}\tilde{V}^*x \\
            x &= \tilde{V}\tilde{\Sigma}^{-1}\tilde{U}^*b
        \end{align*}
        Where,
        \begin{conditions}
            \tilde{\Sigma} & $\Vert a \Vert_2$ \\
        ...
        \end{equation*}
        \item \textbf{Note:} If $a$ is a column vector then $u = \frac{a}{\Vert a \Vert_2}$ because of the following:
        \begin{align*}
            \Vert a \Vert_2 &= \Vert \sigma u \Vert_2 \\
            &= |\sigma| \Vert u \Vert_2
        \end{align*}
        Because $\Vert u \Vert_2 = 1$
        \begin{align*}
            \Vert a \Vert_2 = |\sigma|
        \end{align*}
        Therefore,
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            \item \textbf{From ChatGPT:} PCA finds directions of maximum variance in the data. SVD, on the other hand, decomposes the data matrix to reveal important latent relationships between input (features) and output (samples), with singular values quantifying their importance.
            \item \textbf{From ChatGPT:} PCA seeks to transform the data into a coordinate system where the axes are the Principle components (PC) which are aligned with directions of maximum variance. The PCs themselves are linear combinations of the original variables that make up the data.
            \begin{itemize}
                \item When we say linear combination, we mean the following:
                \begin{equation*}
                    PC_1 = a_1x_1 + a_2x_2 + \hdots + a_nx_n
                \end{equation*}
                where $x_n$ represents the original variable, and $a_n$ are the coefficients (aka. loadings) that are determined using the PCA algorithm. \hl{This is why below we state the PCs are eigenvectors of the covariance matrix :).}
            \end{itemize}
        \end{itemize}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \Bar{x}_j = \frac{1}{n}\Sigma_{i=1}^n X_{ij}
        \end{equation*}
        ...
        \textit{The reason we use $n-1$ (known as \textbf{Bessel's Correction}) is because the above steps to determine the sample variance does not account for the variance of $\Bar{X}$ (sample mean) against the true mean. When working with the population data set one should use $\frac{1}{n}$ rather than $\frac{1}{n-1}$.}
        ...
        \item The PCs (principle components) are the eigenvectors of $C$ (which are the column vectors of $V$) and they define a change in coordinates.
        \begin{equation*}
            CV = VD \Rightarrow C = VDV^* \Rightarrow D = V^*CV
        \end{equation*}

        Plugging the equation above related $B$ to $C$ and using SVD ($B = U\Sigma V^*$), one can derive the following:
        \begin{align*}
            C &= \frac{1}{n-1}B^*B \\
            &= \frac{1}{n-1}V\left(\Sigma \right)^2V^* \\
            D &= \frac{1}{n-1}\left(\Sigma \right)^2
        \end{align*}

        One can relate the singular values to the variance from the diagonal matrix $D$ using the following equation:
        \begin{equation*}
            \lambda_k = \frac{\sigma_k^2}{n-1}
        \end{equation*}

        \item One can scale $B$ to form $\tilde{B}$ so that $\sigma_k^2 = \lambda_k$ \textit{(Great for when coding this stuff)}:

        \href{https://chatgpt.com/c/68338638-bfb4-800a-9865-2d47ea8773f7}{\textbf{Link for More info}}
        \begin{itemize}
            \item When working with the population dataset:
            \begin{equation*}
                C = \frac{1}{n}B^*B = \tilde{B}^*\tilde{B} = V(\Sigma)^2V^*
            \end{equation*}
            Where $\tilde{B} = \frac{1}{\sqrt{n}}B$ because $\left(\frac{1}{\sqrt{n}}B\right)^*\left(\frac{1}{\sqrt{n}}B\right) = \frac{1}{n}B^*B$
            \item When working with the sample dataset:
            \begin{equation*}
                C = \frac{1}{n-1}B^*B = \tilde{B}^*\tilde{B} = V(\Sigma)^2V^*
            \end{equation*}
            Where $\tilde{B} = \frac{1}{\sqrt{n-1}}B$ because $\left(\frac{1}{\sqrt{(n-1)}}B\right)^*\left(\frac{1}{\sqrt{(n-1)}}B\right) = \frac{1}{(n-1)}B^*B$
        \end{itemize}
...
\subsubsection*{Noisy Gaussian Data Example:}

\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/noisy_gaussian_data_example.png}
    \caption{Output from Code}
    \label{fig:Noisy-Gaussian-Data-Ex}
\end{figure}
...
\subsubsection*{Ovarian Cancer Example}
...
\begin{figure}[hbt!]
    \centering
    \includegraphics[width=0.5\linewidth]{Figures/Ovarian_Cancer.png}
    \caption{Plot of the Ovarian Cancer Dataset}
    \label{fig:Plot_Ovarian_Cancer_Example}
\end{figure}

\subsubsection*{Eigenface Example}
...
\begin{itemize}
    ...
    In other words, rotating, stretching, compressing, or translating the vectors in matrix $X$ (with regards to the equation $X = U\Sigma V^*$) impacts the matrices $U$, $\Sigma$, and $V$. Specifically, rotations can impact the left and right singular matrices ($U$ and $V$ respectively), while stretching or compressing (changing the magnitude) the vectors of matrix $X$ can cause changes to $\Sigma$.
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \begin{equation*}
            \left < \mathbb{A}, \mathbb{B}\right> = \Sigma_i a_ib_i
        \end{equation*}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            ...
            \begin{equation*}
                \mathcal{X} = \Sigma^R_{r=1}  a_r \circ b_r \circ c_r
            \end{equation*}
        ...
        \begin{equation*}
            \mathbb{M} = \Sigma^R_{r=1} \lambda_r ma_r^{(1)} \circ ma_r^{(2)} \circ \cdots \circ ma_r^{(N)}
        \end{equation*}
        where 
        \begin{conditions}
            \circ & outer product \\
...
\begin{itemize}
    ...
    \begin{equation*}
        \frac{b-a}{n-1}\left<f(x), g(x) \right> = \Sigma_{k=1}^n f(x_k)\Bar{g}(x_k)\Delta x
    \end{equation*}
    ...
    \item The set of all functions with bounded norm defines the set of \textbf{square integrable functions}($L^2$), which is one of the \href{https://www.youtube.com/watch?v=Fb2ei6lD-d8}{\textbf{Lebesgue integrable functions}}
    \begin{itemize}
        ...
        \item \href{https://chatgpt.com/c/68449325-132c-800a-a44a-c16de2d9b362}{\textbf{Chat GPT} discussion on the square integrable functions.}
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The Fourier series uses the Lebesgue integrable functions over the range of $-\pi$ to $\pi$ (in other words, $L^2[-\pi, \pi]$). 
    \end{itemize}
    \item Example: If $f(x)$ is $2\pi$-periodic, it can be written as:
    \begin{equation*}
        f(x) = \frac{a_0}{2} + \Sigma^\infty_{k=1} (a_kcos(kx)+ b_ksin(kx))
    \end{equation*}
    Where:
    \begin{equation*}
    ...
    \begin{equation*}
        f(x) = \frac{a_0}{2} + \Sigma^\infty_{k=1}\left(a_k cos\left(\frac{2\pi kx}{L} \right) + b_k sin\left(\frac{2\pi kx}{L} \right)\right)
    \end{equation*}
    Where:
    \begin{equation*}
    ...
    \end{itemize}
    \begin{align*}
        f(x) &= \Sigma^{\infty}_{k=-\infty}c_ke^{ikx} \\
        &= \Sigma^{\infty}_{k=-\infty} (a_k + ib_k)(cos(kx) + isin(kx))\\
        &=(a_0 + ib_0)\Sigma^{\infty}_{k=1} \left[ (a_{-k} + a_k)cos(kx) +(b_{-k} - b_k)sin(kx) \right] + i\Sigma^{\infty}_{k=1}  \left[ (b_{-k} + b_k)cos(kx) +(a_{-k} - a_k)sin(kx) \right]\\
    \end{align*}
    ...
    \item Example:
    \begin{equation*}
        f(x) = \Sigma_{k=-\infty}^{\infty} c_k\psi_k(x) = \frac{1}{2\pi} \Sigma_{k=-\infty}^{\infty} \left<f(x), \psi_k (x) \right>psi_k (x)
    \end{equation*}
...
\begin{itemize}
    ...
    \begin{equation*}
        f(x) = \frac{a_0}{2}+\Sigma_{k=1}^\infty \left[ a_kcos\left(\frac{k\pi x}{L}\right) + b_ksin\left(\frac{k\pi x}{L}\right) \right] = \Sigma_{k=-\infty}^\infty c_ke^{i2\pi kx/L}
    \end{equation*}
    ...
    \begin{equation*}
        f(x) = lim_{\Delta w \rightarrow 0} \Sigma_{k=-\infty}^{\infty} \frac{\Delta w}{2\pi} \left[\int^{\pi/\Delta w}_{-\pi/\Delta w} f(\zeta)e^{-ik\Delta w\zeta} d\zeta \right] e^{ik\Delta wx}
    \end{equation*}
    where, 
    \begin{equation*}
...
\begin{itemize}
    ...
    \begin{equation*}
        \Hat{f}_k = \Sigma^{n-1}_{j=0} f_je^{-i2\pi jk/n}
    \end{equation*}
    \item The formulation of the inverse DFT (iDFT):
    \begin{equation*}
        f_k = \frac{1}{n}\Sigma^{n-1}_{j=0}\Hat{f}_je^{i2\pi jk/n}
    \end{equation*}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{itemize}
            ...
            \end{enumerate}
        \end{itemize}
    \item \textbf{From ChatGPT:} The concept of \textbf{multi-resolution analysis} is a technique in signal processing where a signal is analyzed at multiple levels of resolution or scale, allowing one to analyze the signal's global structure and finer details. This is shown as (d) in Figure \ref{fig:Uncertainty Principle FT} while (c) is a visual representation of the STFT (aka. Gabor transform).
    \begin{figure}[hbt!]
        \centering
        \includegraphics[width=0.5\linewidth]{Figures/Uncertainty_principle_Fourier_analysis.png}
        \caption{Representation of the uncertainty principle in the context of Fourier analysis}
        \label{fig:Uncertainty Principle FT}
    \end{figure}
    \end{itemize}
    \item Basic idea around how \href{https://chatgpt.com/c/68322139-e808-800a-82fc-2ab7dbf0e8f2}{\textbf{Wavelet Analysis}} work:
    \begin{enumerate}
    ...
    \begin{enumerate}
        \item Equation to start with:
        \begin{equation*}
            W_{\psi}(f)(a,b) = \left< f, \psi_{a,b} \right> = \int_{-\infty}^{\infty}f(t)\bar{\psi}_{a,b}(t)dt
        \end{equation*}
        Where, $\bar{\psi}_{a,b}(t)$ is the complex conjugate of $\psi_{a,b}(t)$.
        \begin{itemize}
            \item The above equation only works for functions $\psi(t)$ that abides the following property
            \begin{equation*}
                C_{\psi} = \int_{-\infty}^{infty} \frac{|\hat{\psi}(\omega)|^2}{|\omega|}d\omega < \infty
            \end{equation*}
        \end{itemize}
        \item The inverse CWT (iCWT) equation is the following:
        \begin{equation*}
            f(t) = \frac{1}{C_\psi}\int_{-\infty}^{\infty}\int_{-\infty}^{\infty} W_{\psi}(f)(a,b)\psi_{a,b}(t)\frac{1}{a^2}dadb
        \end{equation*}
        \textit{New wavelets can be formed through the convolution $\psi * \phi$ if $\phi$ is a bounded an integrable function}
    \end{enumerate}
    \item Discrete Wavelet Transform (DWT):
    \begin{enumerate}
        \item Equation:
        \begin{equation*}
            W_\psi(f)(j,k) = \left< f, \psi_{j,k} \right> = \int_{-\infty}^{\infty}f(t)\bar{\psi}_{j,k}(t)dt
        \end{equation*}
        where $\psi_{j,k}$ is a family of wavelets, which follows the equation shown below:
        \begin{equation*}
            \psi_{a, b}(t) = \frac{1}{a^j}\psi\left( \frac{t-kb}{a^j} \right)
        \end{equation*}
        \item the inverse DWT (iDWT) equation:
        \begin{equation*}
            f(t) = \Sigma^\infty_{j,k =-\infty} \left<f(t), \psi_{j,k}\right>\psi_{j,k}(t)
        \end{equation*}
        \textit{Explicit computation of DWT is pretty involved...}
    \end{enumerate}
\end{itemize}

\subsection*{Two-Dimensional Transforms and Image Processing}

\begin{itemize}
    \item How does 2-D FFT work?
    \begin{itemize}
        \item For $X \in \mathbb{R}^{n\times m}$ to perform 2-D Fourier transform, one would first need to perform the 1-D transform on each row. This is because you are treating each row in the matrix as a 1-D signal.
        \item From the resulting matrix, one would perform FFT on each column by considering each column to be a 1-D signal.
        \item This will provide frequency information for both vertical and horizontal directions.
        \item An example of 2-D FFT is image processing.
    \end{itemize}
    \item How does higher-dimensional DWT work?
    \begin{itemize}
        \item Similarly to FFT, for 2-D DWT one would perform the multi-resolution decomposition for each row in the matrix, then for each column (for $X \in \mathbb{R}^{n\times m}$).
        \item in DWT, each level forms four quadrants. The diagram is shown below. The levels are not associated with the dimensions. As one goes up levels, the DWT is taken place for only the ``\textbf{LL}'' region.
        \begin{center}
            \begin{tikzpicture}[scale=1.5]
              % Draw the main square
              \draw[thick] (0,0) rectangle (4,4);
            
              % Horizontal and vertical lines to divide into 4 subbands
              \draw[thick] (2,0) -- (2,4);
              \draw[thick] (0,2) -- (4,2);
              ...
              % Optional: add frequency direction indicators
              \node[rotate=90] at (-0.3,2) {Low Pass $\longrightarrow$ High Pass (rows)};
              \node at (2,-0.5) {Low Pass $\longrightarrow$ High Pass (columns)};
            \end{tikzpicture}
        \end{center}
        \item Comparing Level 1 and Level 2 DWT:
        \begin{center}
            \begin{tikzpicture}[scale=1.2]
            
              % -------- Level 1 --------
              % Outer square for level 1
              \draw[thick] (0,0) rectangle (4,4);
              ...
              % Labels level 1
              \node at (1,3) {\textbf{LL$_1$}};
              \node at (3,3) {\textbf{LH$_1$}};
              \node at (1,1) {\textbf{HL$_1$}};
              \node at (3,1) {\textbf{HH$_1$}};
            
              % Title Level 1
              \node at (2,-0.7) {\textbf{Level 1 2D DWT}};
            
              % -------- Level 2 --------
              % Shift right by 5 units for level 2
              \begin{scope}[xshift=6cm]
            
                % Outer square for level 2 (same size)
                \draw[thick] (0,0) rectangle (4,4);
            
                % Level 1 divisions (same as before)
                \draw[thick] (2,0) -- (2,4);
                \draw[thick] (0,2) -- (4,2);
            
                % Level 1 detail labels (unchanged)
                \node at (3,3) {\textbf{LH$_1$}};
                \node at (3,1) {\textbf{HH$_1$}};
                \node at (1,1) {\textbf{HL$_1$}};
            
                % Inner square for LL1 (top-left quadrant)
                \draw[thick] (0,2) rectangle (2,4);
            
                % Level 2 divisions inside LL1
                \draw[thick] (1,2) -- (1,4);
                \draw[thick] (0,3) -- (2,3);
            
                % Level 2 labels inside LL1
                \node at (0.5,3.5) {\textbf{LL$_2$}};
                \node at (1.5,3.5) {\textbf{LH$_2$}};
                \node at (0.5,2.5) {\textbf{HL$_2$}};
                \node at (1.5,2.5) {\textbf{HH$_2$}};
            
                % Title Level 2
                \node at (2,-0.7) {\textbf{Level 2 2D DWT}};
              \end{scope}
...
%Provide information on 2-dimensional FFT and DWT 
\subsubsection*{Example Code}


\section{Sparsity and Compressed Spacing}

\subsection*{Sparsity and Compression}

\begin{itemize}
    \item Most common signals in our world are highly compressible. In other words, when representing the signal in its appropriate basis, only a few modes are active. Thus, less information needs to be stored.
    \begin{itemize}
        \item A basis in this context is a set of vectors that can represent the signal (they are essentially the set of vectors/functions that can represent any other vector/function within the vector/function space).
        \begin{equation*}
            x(t) =  \Sigma_i \alpha_i\phi_i(t)
        \end{equation*}
        where,
        \begin{conditions}
            \phi_i(t) & basis functions \\
            \alpha_i & coefficients to provide weights to each basis function \\
        \end{conditions}
        You are decomposing the signal x into a linear combination of basis vectors (the columns of $\psi$), with weights or coefficients given by $s$.
        
        \item in the context of highly compressible signals, most of the $\alpha_i$ values are equal to 0, with a few significantly large. This enables one to represent a compressible signal as a sparse matrix.
        \begin{equation*}
            x = \Psi s
        \end{equation*}
        where,
        \begin{conditions}
            \Psi & The matrix containing the basis vectors ($\Psi = \left[\psi_i, \psi_2, \hdots, \psi_3\right]$) \\
            s & sparse vector containing the coefficients
        \end{conditions}
        \item Specifically, $s$ is a $K$-sparse in $\Psi$ if there are $K$ non-zero elements within $\Psi$. On the other hand, if $\Psi$ is generic, then only a few active terms within $s$ need to be defined to determine $x$.
        \begin{itemize}
            \item The Fourier modes and wavelets are \textbf{generic or universal basis}. This enables all signals (image, audio, etc) to be compressible.
        \end{itemize}
    \end{itemize}
\end{itemize}

\subsubsection*{Ex: Image Compression V2}

%Need to add the code here...

\subsubsection*{Why Signals are Compressible: The Vastness of Image Space}

\begin{itemize}
    \item Natural images are extremely rare in the vastness of the \textbf{image space}. Therefore, the number of dimensions needed to encode the natural images (essentially non-random images or images that represent the objects we may see in our world) is small, and thus the raw data is highly compressible.
    \begin{figure}[hbt!]
        \centering
        \includegraphics[width=0.4\linewidth]{Figures/image_space_and_natural_images.png}
        \caption{Space of natural images vs the total image space}
        \label{fig:image-space-vs-natural-images-space}
    \end{figure}
\end{itemize}
...
\begin{itemize}
    \item This methodology works by collecting a few compressed or random measurements and inferring the sparse representation under the transformed basis. The challenge is that finding the sparsest vector is an NP (non-polynomial) hard problem.
    \item Mathematically, this method works by utilizing the sparsity of a matrix under a generic basis to achieve full reconstruction while only using a few measurements.
    \begin{itemize}
        \item Example: If $x$ is $K$-sparse (aka. $K$ non-zero elements) in $\psi$ then instead of measuring $x$ directly through $n$ measurements, it would be easier to collect less random and/or compressed measurements and work out what the no-zero values in $x$ are.
        \begin{equation*}
            y = Cx
        \end{equation*}
        Where $C \in \mathbb{R}^{p\times n}$ and $K < p << n$ (p is the amount of linear measurements on state $x$). The measurement matrix $C$ holds the set of $p$ linear measurements on state $x$.
        \item Because $x = \Psi s$ one can derive the following:
        \begin{align*}
            y &= Cx \\ 
            &= C\Psi s
        \end{align*}
        \item One can also write $C\Psi$ as $\Theta$ to get the following equation:
        \begin{equation*}
            y = \Theta s
        \end{equation*}
        \item In the equation $y = \Theta s$ there are infinitely many solution for $s$. to determine the sparsest solution $\hat s$, one should use the following equation:
        \begin{align*}
            \hat s = argmin_s \Vert s \Vert_0, & \text{ subject to } y= C\Psi s
        \end{align*}
        where $l_0$ norm (or $\Vert \cdot \Vert_0$) equals the number of non-zero entries. Also referred to as the cardinality of the vector.
        \item The above equation becomes \textbf{intractable} (it cannot be solved efficiently; in other words, it can't be solved in polynomial time) at moderately large $n$ or $K$. However, under certain conditions for $C$, one can relax the optimization to a convex $l_1$-minimization:
        \begin{equation*}
            \hat s = argmin_s \Vert s \Vert_1 \text{    subject to  } y= C\Psi s
        \end{equation*}
        where,
        \begin{equation*}
            \Vert s \Vert_1 = \Sigma^n_{k=1} |s_k|
        \end{equation*}
        \textit{The $l_1$-norm is also known as the taxi-norm or manhattan-norm.}
        \item The conditions that $C$ needs to be in to use the $l_1$-minimization is the following:
        \begin{enumerate}
            \item $C$ must be incoherent wrt the sparsifying basis. In other words, the rows of $C$ are not correlated with the columns of $\Psi$.
            \item The number of measurements, $p$, must be sufficiently large. Must be in the order of:
            \begin{equation*}
                p \approx O\left(Klog\left(\frac{n}{K}\right)\right)\approx k_1Klog\left(\frac{n}{K}\right)
            \end{equation*}
            where $k_1$ is dependent on how incoherent $C$ and $\Psi$ are.
        \end{enumerate}
    \end{itemize}
\end{itemize}

\subsection*{Compressed Sensing Examples}

%% Need to implement when I work on practice problems

\subsection*{The Geometry of Compression}
