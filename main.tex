...
\begin{itemize}
    \item A interesting, yet useful, property of SVD is when you perform a left or right unitary transformation (an example is discrete fourier transform, or $\mathcal{F}$) to either the $U$ or $V$ matrices, it does not impact the terms in the SVD.
...
\begin{itemize}
    ...
    \begin{equation*}
        \mathcal{E}(\Vert X - QY \Vert_2) \leq \left( 1 + \sqrt{\frac{r}{p-1}} + \frac{e\sqrt{r+p}}{p}\sqrt{m-r} \right)^{1/(2q+1)}\sigma_{k+1}(X)
    \end{equation*}
    Where,
    \begin{conditions}
        e & Euler number
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item \textbf{Structured Random Projections} can also provide efficient sketches and bring down computational costs to $\mathcal{O}(nm\text{log}(r))$
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Say that we have two matrices: $A \in R^{I\times K}$ and $B \in R^{J\times K}$. Their \textbf{Khatri-Rao product} is denoted by $A 
        \odot B$
        \begin{itemize}
        ...
        \item The inner product of two N-way tensors (for example, $\mathcal{A}$ and $\mathcal{B}$) is shown below
        \begin{equation*}
            \left < \mathcal{A}, \mathcal{B}\right> = \sum_i a_ib_i
        \end{equation*}
        Where: $\mathcal{A}$ is of size $I_1\times I_2 \times \cdots \times I_N$ and $i = (i_1, i_2, \cdots, i_n)$

        \item The Frobenius norm of a tensor A (denoted as $\Vert \mathcal{A} \Vert_F$) is the square root of the inner product.
        \begin{equation*}
            \Vert \mathcal{A} \Vert_F = \sqrt{\left< \mathcal{A}, \mathcal{A}\right>}
        \end{equation*}

        \item The mode-n matricization (or unfolding of tensor $\mathcal{A}$) is denoted by $mA_{(n)}$
    \end{enumerate}
